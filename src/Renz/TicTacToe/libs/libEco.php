...
namespace Renz\TicTacToe\libs;

use Closure;
use cooldogedev\BedrockEconomy\BedrockEconomy;
use cooldogedev\BedrockEconomy\api\BedrockEconomyAPI;
use cooldogedev\BedrockEconomy\libs\cooldogedev\libSQL\context\ClosureContext;
use Renz\TicTacToe\libs\Utils;
...
final class libEco
{
    ...
    private static function getEconomy(): array
    {
        ...
        if ($api !== null) {
            ...
            if ($api !== null) {
                return [Utils::BEDROCKECONOMYAPI, BedrockEconomyAPI::getInstance()];
            } else{
                return [null];
            }
        }
    ...
    public static function reduceMoney(Player $player, int $amount, Closure $callback): void
    {
        if (self::getEconomy()[0] === Utils::ECONOMYAPI) {
            ...
            BedrockEconomyAPI::getInstance()->subtractFromPlayerBalance($player->getName(), (int) ceil($amount), ClosureContext::create(static function (bool $success) use ($callback): void {
                $callback($success);
            }));
        }
    }
